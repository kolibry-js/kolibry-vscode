...
export function configEditor() {
  ...
  window.createTreeView('kolibry-slides', {
    treeDataProvider: provider,
    showCollapseAll: true,
  })

  commands.registerCommand('kolibry.goto', async (idx: number) => {
    revealSlide(idx)
    previewProvider.updateSlide(idx)
  })

  commands.registerCommand('kolibry.next', async () => {
    const editor = window.activeTextEditor
  ...
  commands.registerCommand('kolibry.prev', async () => {
    const editor = window.activeTextEditor
    if (!editor || editor.document !== ctx.doc)
      return
  ...
  commands.registerCommand('kolibry.move-up', async (item: SlideItem) => {
  ...
  commands.registerCommand('kolibry.move-down', async (item: SlideItem) => {
  ...
  commands.registerCommand('kolibry.preview-refresh', previewProvider.refresh.bind(previewProvider))

  commands.registerCommand('kolibry.markdown-to-html', async () => {
    const editor = window.activeTextEditor
    const doc = editor?.document
    if (!editor || !doc)
      return
